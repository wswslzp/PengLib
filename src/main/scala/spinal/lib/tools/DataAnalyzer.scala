...
/** Base type data analyzer. It provides some utilities that help designer
  * analyze the circuit at the runtime.
  * @param data
  *   \- the data to be analyzed. The analyze unit is BaseType signal like
  *   Bits/UInt/SInt/Bool
  */
class DataAnalyzer(data: BaseType) {
  ...
  /** Return all the fan-ins signals of this data
    * @return
    *   \- a set of fan-ins signals.
    */
  def allFanIn: Set[BaseType] = {
    val ret = Set.newBuilder[BaseType]
    data.foreachStatements { st =>
      if (
        !(st.isInstanceOf[InitAssignmentStatement] || st
          .isInstanceOf[InitialAssignmentStatement])
      ) {
  ...
  /** Filter the fan-ins according to the condition.
    * @param cond
    *   \- a predicate to filter the fan-in
    * @return
    *   a set of fan-ins signals
    */
  ...
  def walkFanIn(cond: BaseType => Boolean)(func: BaseType => Unit): Unit =
    getFanIn(cond).foreach(func)

  /** Return all the fan-outs signals of this data
    * @return
    *   \- a set of fan-outs signals.
    */
  ...
  /** Filter the fan-outs according to the condition.
    * @param cond
    *   \- a predicate to filter the fan-out
    * @return
    *   a set of fan-outs signals
    */
  def getFanOut(cond: BaseType => Boolean): Set[BaseType] =
    allFanOut.filter(cond)
